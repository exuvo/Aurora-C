...
namespace shaderAPI
{
    ...
    //Public
    ShaderAPI::ShaderAPI(VkInstance instance, VkDevice logicalDevice, VkPhysicalDevice physicalDevice, VkRenderPass renderPass, VkExtent2D windowSize)
    {
        ...
        this->createVulkanIndexBuffer();
        this->createVulkanVertexBuffer();

        this->createGraphicsPipeline();
    }

    void ShaderAPI::draw(VkCommandBuffer commandBuffer)
    {
    ...
    void ShaderAPI::clear()
    {
        this->cleanup();
    }

    //Private
    void ShaderAPI::createVulkanVertexBuffer()
    {
        VkDeviceSize bufferSize = sizeof(vertices[0]) * vertices.size();

        createBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, this->vertexBuffer, this->vertexBufferMemory, this->logicalDevice, this->physicalDevice);
    ...
    void ShaderAPI::createVulkanIndexBuffer()
    {
        VkDeviceSize bufferSize = sizeof(indices[0]) * indices.size();

        createBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, this->indexBuffer, this->indexBufferMemory, this->logicalDevice, this->physicalDevice);

        void *data;
        vkMapMemory(this->logicalDevice, this->indexBufferMemory, 0, bufferSize, 0, &data);
        memcpy(data, indices.data(), (size_t)bufferSize);
        vkUnmapMemory(this->logicalDevice, this->indexBufferMemory);
    }

    void ShaderAPI::createGraphicsPipeline()
    {
        ...
        VkShaderModule vertShaderModule = createShaderModule(vertShaderCode, this->logicalDevice);
        VkShaderModule fragShaderModule = createShaderModule(fragShaderCode, this->logicalDevice);
        ...
        viewport.x = 0.0f;
        viewport.y = 0.0f;
        viewport.width = (float)this->windowSize.width;
        viewport.height = (float)this->windowSize.height;
        viewport.minDepth = 0.0f;
        viewport.maxDepth = 1.0f;

        VkRect2D scissor{};
        scissor.offset = {0, 0};
        scissor.extent = this->windowSize;
    ...
    void ShaderAPI::cleanupResizing()
    {
